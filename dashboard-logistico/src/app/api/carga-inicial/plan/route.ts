...
async function requireLecturaPlan() {
  const porAvance = await requireAdminPermission("CI-Avance");
  if (porAvance.autorizado) return porAvance;
  return requireAdminPermission("CI-Carga");
}
...
export async function GET() {
  ...
  try {
    ...
    if (error) throw new Error(`Supabase (plan_carga_inicial): ${error.message}`);

    return NextResponse.json({ success: true, plan: (data as PlanCargaInicialRow | null) ?? null });
  } catch (err) {
...
export async function POST(request: NextRequest) {
  ...
  const auth = await requireAdminPermission("CI-Carga");
  if (!auth.autorizado) {
    return NextResponse.json({ success: false, error: auth.error }, { status: auth.status });
  }
