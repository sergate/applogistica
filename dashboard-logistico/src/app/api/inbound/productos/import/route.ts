...
// -----------------------------------------------------------------------
// Import independiente del detalle de productos por legajo (mismo menú que
// /api/inbound/import, pero tabla y archivo separados -- no se actualiza
// con la misma frecuencia). Mismo patrón de reemplazo por "legajo": antes
  ...
  try {
    ...
    if (esPrimerLote && Array.isArray(legajosAEliminar) && legajosAEliminar.length > 0) {
      for (let i = 0; i < legajosAEliminar.length; i += DELETE_CHUNK) {
        ...
        const { error } = await supabaseAdmin.from("inbound_productos").delete().in("legajo", chunk);
        if (error) {
          throw new Error(`Supabase (inbound_productos - borrado por legajo): ${error.message}`);
        }
      }
    }
    ...
    if (error) {
      throw new Error(`Supabase (inbound_productos - insert): ${error.message}`);
    }
